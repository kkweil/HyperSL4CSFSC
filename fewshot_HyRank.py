...
class HyperDataset(Dataset):
    def __init__(self, data, label, wave, is_train):
        self.data = data
        self.label = label
        self.wave = wave
        self.is_train = is_train
        self.enable_superpixels = True
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_superpixels:
            data_, pos, mask, _ = hyperspectral_superpixels(data)
        else:
...
def parse_txt_file(filepath):
    ...
    # 提取 wavelength 部分（花括号里的数）
    wave_str = re.search(r"wavelength\s*=\s*{([^}]*)}", content, re.S)
    if not wave_str:
        raise ValueError("未找到 wavelength 部分")
    # 转换为浮点数列表
    wavelength = [float(x) for x in wave_str.group(1).replace("\n", " ").split(",") if x.strip()]

    # 提取 bbl 部分
    bbl_str = re.search(r"bbl\s*=\s*{([^}]*)}", content, re.S)
    if not bbl_str:
        raise ValueError("未找到 bbl 部分")
    bbl = ast.literal_eval("{" + bbl_str.group(1) + "}")  # 转成 set of tuples
    bbl = sorted(list(bbl))  # 转成有序列表

    return wavelength, bbl


def get_wavelength_by_bbl(wavelength, bbl):
    result = []
    for start, end in bbl:
        # 注意索引从1开始，所以要-1
        result.extend(wavelength[start-1:end])
    return np.asarray(result)


def read_Hyrank(data_path, gt_path, wave_path):

    data = loadmat(data_path)
    label = loadmat(gt_path)
    data = data['ori_data']
    label = label['map']
    W, H, C = data.shape
    ...
    wavelength, bbl = parse_txt_file(wave_path)
    wavelength = get_wavelength_by_bbl(wavelength, bbl)
...
if __name__ == '__main__':
    ...
    source_data_path = './data/HyRANK/Dioni.mat'
    source_label_path = './data/HyRANK/Dioni_gt_out68.mat'
    source_wave_path = './data/HyRANK/wavelength.txt'
    
    
    target_data_path = './data/HyRANK/Loukia.mat'
    target_label_path = './data/HyRANK/Loukia_gt_out68.mat'
    target_wave_path = './data/HyRANK/wavelength.txt'

    source_data, source_labels, source_wavelength = read_Hyrank(source_data_path, source_label_path, source_wave_path)
    
    
    from util.color import palette
    rgb_s = palette[source_labels.astype('int')]
    plt.imsave('Dioni_gt_out68.png', rgb_s)
    
    
    target_data, target_labels, target_wavelength = read_Hyrank(target_data_path, target_label_path, target_wave_path)
    ...
    source_x_train, source_x_test, source_y_train, source_y_test, source_coordinate_train, source_coordinate_test\
    = split_train_test(source_data, source_labels, patch_size=patch_size, ratio=5, augment_times=4)
    ...
    # 训练集
    source_train_Dataset = HyperDataset(source_x_train, source_y_train, source_wavelength, is_train=False)
    ...
    # 测试集
    source_test_Dataset = HyperDataset(source_x_test, source_y_test, source_wavelength, is_train=False)
    source_test_DataLoader = DataLoader(source_test_Dataset, batch_size=96, shuffle=False, drop_last=False, pin_memory=True,num_workers=4,persistent_workers=True)
    
    
    target_test_Dataset = HyperDataset(target_x_test, target_y_test, target_wavelength, is_train=False)
    target_test_DataLoader = DataLoader(target_test_Dataset, batch_size=96, shuffle=False, drop_last=False, pin_memory=True, num_workers=4, persistent_workers=True)
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 50 == 0 or epoch==epochs-1:
            ...
            if best_OA < OA:
                torch.save(model.state_dict(), "Hyrank.pth")
                best_OA = OA
                best_AA = AA
                best_aa = aa
                best_Kappa = Kappa
    ...
    # plt.imsave('Dioni_pred.png', rgb_s)
    ...
    # state_dict = torch.load("Hyrank.pth", map_location="cpu")
